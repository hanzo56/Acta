...
import { readBookDinnerScenario } from "../scenarioSession";
import type { ScenarioStepStatus } from "../scenarioGraphStorage";

const STEP_LOAD_MS = 3200;

const DEFAULTS = {
  guest: "Your guest",
  restaurant: "Chef’s choice shortlist",
  cuisine: "Quiet dining room",
  when: "Next available evening",
} as const;
...
export function BookDinnerGraphPage() {
  ...
  const scenario = useMemo(() => {
    const r = readBookDinnerScenario();
    return r ?? { ...DEFAULTS };
  }, []);
  ...
  const steps = useMemo(
    () => [
      {
        title: `Opened thread with ${scenario.guest} and captured dietary prefs`,
        icon: imgMessages,
        iconClass: "size-5",
      },
      {
        title: `Checked both calendars for “${scenario.when}”`,
        icon: imgCalendar,
        iconClass: "h-5 w-[18px]",
      },
      {
        title: `Matched venues to “${scenario.cuisine}” within travel window`,
        icon: imgForkKnife,
        iconClass: "h-5 w-[15px]",
      },
      {
        title: `Held table path via OpenTable — primary pick: ${scenario.restaurant}`,
        icon: imgOpenTable,
        iconClass: "h-5 w-[15px]",
      },
      {
        title:
          "Drafted confirmation + calendar invite (send pending your final OK)",
        icon: imgBell,
  ...
  return (
    <div className="acta-shell text-[#e5e2e1]">
      <main className="acta-graph-body" aria-label="Book dinner graph">
        <div className="mx-auto flex w-full max-w-[672px] flex-col gap-8 px-6 pb-[max(2rem,calc(1.25rem+env(safe-area-inset-bottom,0px)))]">
          <header className="flex flex-col gap-1">
            ...
            <p className="pt-1 text-[14px] font-normal leading-5 text-[#bbcabf]">
              Simulated run using your preview answers — nothing is sent outside
              this demo.
            </p>
          </header>
          ...
          {showFollowUp ? (
            ...
            >
              <p className="text-[16px] font-medium leading-6 text-[#e5e2e1]">
                Should I add a quiet table note and optional pairing budget cap so
                the restaurant holds the right table style?
              </p>
              <p className="mt-3 text-[13px] leading-5 text-[rgba(187,202,191,0.75)]">
                Acta pauses on ambiguous hospitality details instead of guessing.
              </p>
              <div className="mt-5 flex flex-col gap-2 sm:flex-row">
                <button
                  type="button"
                  onClick={() =>
                    resolveFollowUp("Added: quiet booth + pairing capped at $120")
                  }
                ...
                >
                  Yes — quiet booth + pairing cap
                </button>
                <button
                  type="button"
                  onClick={() => resolveFollowUp("Skipped — standard seating")}
